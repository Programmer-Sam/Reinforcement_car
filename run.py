...
class CarEnv(gym.Env):
    ...
    def step(self,action):
        ...
        if self.circle_move==True:
            ...
            self.circle_x+= self.DIRECTION_VECTOR_LOOKUP[self.v][0]*(0.3+self.acceleration)
            self.circle_y+= self.DIRECTION_VECTOR_LOOKUP[self.v][1]*(0.3+self.acceleration)
            if self.acceleration>0.008:
    ...
    def render(self):
        ...
        pygame.draw.line(DS,(255,100,255),(arbitrary[0]),(arbitrary[1]),10)
        pygame.draw.line(DS,(255,100,255),(arbitrary[3]),(arbitrary[2]),10)
        pygame.draw.line(DS,(255,100,255),(arbitrary[3]),(arbitrary[1]),10)
        pygame.draw.line(DS,(200,0,255),(arbitrary[0]),(arbitrary[2]),10)
...
DISCOUNT=0.99
REPEAT=1
table_size=[30]*len(env.observation_space.high)#tweak 20
bins=(env.observation_space.high-env.observation_space.low)/table_size
epsilon=0

load="4600"
q_table= np.load(f"qtables/{load}-qtablePS.npy")

aggr_ep_rewards={'ep':[],'avg':[],'min':[],'max':[]}

def StateToBins(state):
    discreteState=(state-env.observation_space.low)/bins
    return tuple(discreteState.astype(np.int))

for episode in range(REPEAT):
    rewardT=0
    discreteState=StateToBins(env.reset())
    done=False
    while not done:
        x=np.random.random()
        if x>epsilon:
        ...
        newDiscreteState=StateToBins(nextState)
        rewardT+=reward
        env.render()
        if not done:
            max_future_q=np.max(q_table[newDiscreteState])
            current_q=q_table[discreteState+(action,)]
            new_q=(1-lr)*current_q+lr*(reward+DISCOUNT*max_future_q)
            q_table[discreteState+(action,)]=new_q
